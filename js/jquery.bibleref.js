(function() {
  var $, allBooks, allBooksRegEx, book, normalizeRef, pattern, patterns, replacement;
  ...
  normalizeRef = function(t) {
    var name, pat;
    t = t.trim();
    for (name in patterns) {
      pat = patterns[name];
      t = t.replace(RegExp("\\b" + pat + "\\b\\.?"), name);
    }
    t = t.replace(/(\s|&nbsp;)+/g, '+');
    return t;
  };
  ...
  allBooks = '(' + ((function() {
    var _results;
    _results = [];
    for (book in patterns) {
      pattern = patterns[book];
      _results.push("\\b" + pattern);
    }
    return _results;
  })()).join('|') + ')';
  ...
  $(document).ready(function() {
    return $("body").on("mouseover", "p,li,dd,dt,td,q,blockquote", function(e) {
      if ($(this).data("bibleref") === "finished") {
        return;
      }
      $(this).html(function(index, html) {
        return html.replace(allBooksRegEx, replacement);
      });
      return $(this).data("bibleref", "finished");
    });
  });
